...
- Schedule "My Times" cannot save against LocalCheckProd. Root cause is a privilege gap, not client code: `planned_visits` grants to `authenticated` are `SELECT, DELETE` only, so every insert is rejected with `42501` before the correct `planned_visits_insert_self` policy can run, while deletes succeed. `docs/supabase/migrations/20260727_grant_planned_visits_writes.sql` is written but **not applied**; applying it needs Jesse's approval. The client no longer reports an empty or court-less save as success.
