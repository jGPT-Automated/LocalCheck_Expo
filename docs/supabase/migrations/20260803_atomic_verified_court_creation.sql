-- Atomic trusted Add Court insertion. Apply before deploying verify-court.
create or replace function public.create_verified_court(
  p_added_by uuid,
  p_slug text,
  p_name text,
  p_address text,
...
declare
  ...
  v_row public.courts;
begin
  -- Court creation is rare. One transaction-scoped lock makes the quota and
  -- radius checks authoritative even when different users submit together.
  ...
  -- Reuse the nearest established market inside the metro radius. This keeps
  -- suburb city labels (for example Sugar Land) discoverable in Houston while
  -- falling back to the submitted city when LocalCheck enters a new region.
  ...
  insert into public.courts (
    ...
    p_latitude, p_longitude, p_sport_type, p_access_type, p_setting,
...
revoke execute on function public.create_verified_court(uuid, text, text, text, text, text, double precision, double precision, text, text, text, text)
  from public, anon, authenticated;
grant execute on function public.create_verified_court(uuid, text, text, text, text, text, double precision, double precision, text, text, text, text)
  to service_role;
