-- Personalize the 1v1 match-confirmed notification and send it to BOTH
-- participants (not just the submitter). Every "SCORE APPROVED / The
-- result is final and ratings are updated." notification sent since
-- 2026-09-03 was successfully delivered (push ticket accepted, receipt
-- ok) and never opened — the generic text carries no reason to tap in.
-- Personalizing with the actual result and ELO delta is the fix; this
-- does not touch scheduled/team match confirmations, which still send no
-- notification at all (tracked as a known follow-up, not fixed here).
  ...
  if new.status = 'confirmed' and new.run_id is null and new.team_size = 1 then
    ...
    loop
      ...
      perform private.create_notification(
        ...
        case when v_side = new.winner_side then 'YOU WON' else 'FINAL SCORE' end,
        (case when v_side = new.winner_side then 'Beat ' else 'Lost to ' end) || v_opponent_name
          || ' ' || v_my_score::text || '–' || v_their_score::text
          || ' · ELO ' || coalesce(v_elo_after, 0)::text
