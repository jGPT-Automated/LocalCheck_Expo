...
export default function AddCourtRoute() {
  ...
  const reverseGeocode = useCallback(async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      setAddress((value) => value || street);
      setCity((value) => value || place.city?.trim() || place.district?.trim() || "");
      setState((value) => value || normalizeState(place.region));
      setSuggestedOfficialName((value) => value || official);
      setSuggestedShortName((value) => value || compact);
      setOfficialName((value) => value || official);
      setShortName((value) => value || compact);
  ...
  return <View style={styles.root}>
    <View style={[styles.header, { paddingTop: Math.max(12, useSafeAreaInsets().top) }]}>
      <Pressable onPress={() => router.back()} accessibilityLabel="Close add court"><Feather name="x" size={22} color={Colors.text} /></Pressable>
